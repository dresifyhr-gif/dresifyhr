...
export default function AdminLoginPage() {
  ...
  const inputCls =
    "w-full rounded-[12px] border border-[var(--a-line)] bg-[var(--a-surface-2)] px-4 py-3 text-sm text-[var(--a-text)] outline-none transition focus:border-slate-400 focus:bg-[var(--a-card)]";
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--a-surface-2)] px-6">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-[var(--a-line)] bg-[var(--a-card)] p-8 shadow-sm">
        ...
        <input
          ...
          className={inputCls}
        />
        {error && <p className="mt-2 text-xs text-red-500">Kriva prijava.</p>}
        <button
          type="submit"
          disabled={loading}
