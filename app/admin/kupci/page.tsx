...
function CustomerLink({ name, phone }: { name?: string | null; phone?: string | null }) {
  ...
  if (!key) return <span className="text-[var(--a-text)]">{label}</span>;
  return (
    <Link href={`/admin/kupci/${key}`} className="font-medium text-[var(--a-text)] underline decoration-slate-300 underline-offset-2 hover:text-[var(--a-text)] hover:decoration-slate-500">
      {label}
    </Link>
  );
}
    ...
    <AdminShell title="Kupci" subtitle="Najbolji kupci i oni koje treba vratiti">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
              ...
              {m.bestCustomers.map((c) => (
                <li key={c.id} className="flex items-center justify-between text-sm">
                  <span className="min-w-0 truncate">
                    <CustomerLink name={c.name} phone={c.phone} /> <span className="text-[var(--a-text-3)]">· {c.totalOrders}×</span>
                  </span>
        ...
        <Panel title={`Vrati kupce (${m.winbackDays}+ dana bez kupnje) · ${m.inactive.length}`}>
          {m.inactive.length === 0 ? (
            <div className="text-sm text-[var(--a-text-3)]">Nema neaktivnih kupaca.</div>
          ) : (
              ...
              {m.inactive.map((c) => {
                ...
                return (
                  <li key={c.id} className="flex items-center justify-between gap-2 text-sm">
                    <span className="min-w-0 truncate">
                      <CustomerLink name={c.name} phone={c.phone} /> <span className="text-[var(--a-text-3)]">· {c.lastOrderAt.toLocaleDateString("hr-HR")}</span>
                    </span>
      ...
      <div className="mt-5">
        <Panel title={`Rizični kupci (${m.riskMinFailed}+ odbijanja) · ${m.riskyCustomers.length}`}>
          {m.riskyCustomers.length === 0 ? (
            <div className="text-sm text-[var(--a-text-3)]">Nema rizičnih kupaca — svi uredno preuzimaju. 👌</div>
          ) : (
            <ul className="max-h-96 space-y-2 overflow-y-auto">
                ...
                return (
                  <li key={`${c.phone}-${i}`} className="flex items-center justify-between gap-2 text-sm">
                    <span className="min-w-0 truncate">
                      ...
                      {c.phone ? <span className="text-[var(--a-text-3)]"> · {c.phone}</span> : null}
                    </span>
                    <span className="flex shrink-0 items-center gap-2">
      ...
      <div className="mt-5">
        <Panel title="Odakle kupci (top gradovi)">
          {m.topCities.length === 0 ? (
            <div className="text-sm text-[var(--a-text-3)]">Nema podataka.</div>
          ) : (
            <ul className="grid gap-2 sm:grid-cols-2">
              {m.topCities.map((c) => (
                <li key={c.name} className="flex items-center justify-between text-sm">
                  <span className="text-[var(--a-text)]">{c.name}</span>
                  <span className="text-[var(--a-text-2)]">
                    <span className="font-semibold text-[var(--a-text)]">{c.count}</span> · {eur(c.total)}
                  </span>
                </li>
              ))}
            </ul>
