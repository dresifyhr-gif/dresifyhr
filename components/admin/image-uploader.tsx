  ...
  return (
    <div>
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center gap-1 rounded-[12px] border-2 border-dashed p-5 text-center text-sm transition ${drag ? "border-slate-900 bg-[var(--a-surface-2)]" : "border-[var(--a-line)] hover:border-[var(--a-line)]"}`}
      >
        <span className="text-2xl">🖼️</span>
        <span className="font-medium text-[var(--a-text-2)]">{busy ? "Uploadam…" : "Povuci slike ovdje ili klikni za odabir"}</span>
        <span className="text-[11px] text-[var(--a-text-3)]">JPG / PNG / WebP · max 8 MB · prva slika je glavna</span>
        <input
          ref={inputRef}
          type="file"
          accept="image/*"
